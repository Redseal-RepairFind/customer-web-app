...
function toCSSVars(v?: ToastVars): React.CSSProperties {
  ...
  for (const key in v) {
    const cssVar = map[key as keyof ToastVars];
...
export function ToastProvider({
  ...
  const timers = useRef<
    Map<string, { timeout?: any; remaining: number; start: number }>
  >(new Map());
  ...
  const scheduleTimer = useCallback((id: string, duration: number) => {
    ...
    const timeout = setTimeout(() => {
      // play exit animation before removal
      setItems((prev) =>
  ...
  const add = useCallback(
    (
      ...
      defaultVariant: ToastVariant = "info"
    ): string => {
      const base: ToastOptions =
        typeof opts === "string" ? { title: opts } : opts;
      const id = base.id ?? uid();

      // If an item with same id exists, patch it (supports progress updates)
      setItems((prev) => {
      ...
      // (re)schedule auto-dismiss if duration > 0
  ...
  const dismissAnimated = useCallback((id: string) => {
    // mark closing then remove after EXIT_MS
    setItems((prev) =>
      prev.map((i) => (i.id === id ? { ...i, closing: true } : i))
    );
    const t = timers.current.get(id);
  ...
  const clearAll = useCallback(() => {
    setItems([]);
    timers.current.forEach((t) => t.timeout && clearTimeout(t.timeout));
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "Escape" && items.length)
        dismissAnimated(items[items.length - 1].id);
    };
  ...
  return (
    <ToastContext.Provider value={api}>
      {children}
      <Portal>
        {/* apply GLOBAL theme variables here */}
...
function Icon({ variant }: { variant: ToastVariant }) {
  ...
  if (variant === "warning")
    return (
      <svg viewBox="0 0 24 24" className={styles.icon} aria-hidden>
        <path d="M1 21h22L12 2 1 21zm11-3a1.5 1.5 0 110-3 1.5 1.5 0 010 3zm-1-5V9h2v4h-2z" />
      </svg>
    );
  return (
    <svg viewBox="0 0 24 24" className={styles.icon} aria-hidden>
  ...
  return (
    ...
    >
      {items.map((item) => {
        ...
        // Full custom renderer => you control inner markup
        if (item.render) {
          ...
          return (
            <li
              ...
              style={{ ...toCSSVars(item.vars), ...item.style }}
        ...
        // Default chrome + optional content slot
        return (
          <li
            key={item.id}
            role={role}
